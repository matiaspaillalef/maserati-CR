...
class CircularAudioWave {
            ...
            series: [{
                    ...
                    lineStyle: {
                        color: {
                            colorStops: [{
                                    offset: 0.7,
                                    color: 'white' // linea derecha - #e91e63
                                },
                                {
                                    offset: 0.3,
                                    color: 'white' // linea izquierda - #3f51b5
                                }
                            ],
                        },
                        shadowColor: 'transparent', // sombra de la linea - blue
                        shadowBlur: 10,
                    },
                    zlevel: 2,
                ...
                },
                {
                    ...
                    showSymbol: false,
                    lineStyle: {
                        color: 'transparent', // linea exterior - #87b9ca
                        shadowColor: 'transparent', // sombra de la linea exterior - #87b9ca
                        shadowBlur: 10,
                    },
                    data: Array.apply(null, {
                        length: 361
                ...
                },
                {
                    ...
                    itemStyle: {
                        color: {
                            ...
                            colorStops: [{
                                offset: 0,
                                color: 'transparent' // gradiente radial circulo interno - #87b9ca
                            }, {
                                offset: 1,
                                color: 'transparent' // frondo del circulo interno - white
                            }],
                        },
                    },
                    silent: true,
    ...
    play() {
        if (this.sourceNode && this.sourceNode.buffer) {
            ...
            this._debouncedDraw();
            document.body.classList.add('play-audio');
        }
    }
    ...
    // time: Numero de milisegundos que durará la transición
    // steps: Números de pasos en que se ejecutara la transición
    stop(time = 1500, steps = 20) {
        if (time <= 0 || steps <= 0) {
            time  = 0;
            steps = 1;
        }
        ...
        if (this.playing && !this.stopping) {
            this.stopping = true;
            this._stopping(time, steps, 0);
        }
        document.body.classList.remove('play-audio');
    }
    
    _stopping(time, steps, step) {
        if (!this.playing) {
            return;
        }

        step += ( 1 / steps );
        let newValue = 1 - EasingFunctions.easeOutQuad(step);

        if ( step >= 1 || newValue <= 0 ) {
            this.gainNode.gain.value = 0;
            this.sourceNode.stop(0);
        }
        else {
            this.gainNode.gain.value = newValue;
            setTimeout(() => { this._stopping(time, steps, step) }, Math.round( time / steps ));
        }
    ...
    // TODO: Allow callback
    onended() {
        if (!this.opts.loop || this.stopping) {
            this.playing = false;
            this.stopping = false;
